...
- O laboratório concluído **Implementar e gerenciar o armazenamento do AVD (AD DS)**
...
1. No painel do **Cloud Shell**, execute o seguinte para iniciar as VMs do Azure host da sessão da Área de Trabalho Virtual do Azure que você usará neste laboratório:
...
1. No painel do **Cloud Shell**, execute o seguinte para habilitar a Comunicação Remota do PowerShell nos Hosts de Sessão.
...
1. Na folha **az140-21-p1-0**, selecione **Conectar**, no menu suspenso, selecione **Conectar via Bastion**.
...
1. Dentro da sessão Bastion para **az140-21-p1-0**, inicie o Microsoft Edge, navegue até a [página de download do FSLogix](https://aka.ms/fslogix_download), baixe os binários de instalação compactados do FSLogix, extraia-os para a pasta **C:\\Allfiles\\Labs\\04** (crie a pasta, se necessário), navegue até a subpasta **x64\\Versão**, clique duas vezes no arquivo **FSLogixAppsSetup.exe** para iniciar o assistente do **Microsoft FSLogix Apps Setup** e instale o Microsoft FSLogix Apps com as configurações padrão.
   ...
   > **Observação**: A instalação do FSLogix não será necessária se a imagem já a incluir.

1. Na sessão do Bastion para **az140-21-p1-0**, inicie o **ISE do Windows PowerShell** como administrador.
1. Do **Administrador: Console ISE do Windows PowerShell**, execute o seguinte para instalar a versão mais recente do módulo do Az PowerShell (insira **Y** quando solicitado a instalar e importar o NuGet):
...
1. No console **Usuários e Grupos Locais**, na lista de grupos, clique duas vezes no grupo **Lista de Exclusão do Perfil do FSLogix**, observe que ele não inclui membros de grupo por padrão e selecione **OK** para fechar a janela **Propriedades** do grupo. 
   ...
   > **Observação**: A etapa a seguir não será necessária se o FSLogix já estiver instalado nos hosts de sessão.
...
1. Na sessão do Bastion para **az140-21-p1-0**, clique com o botão direito do mouse em **Iniciar**, no menu de clique com o botão direito do mouse, selecione **Desligar ou sair** e, no menu em cascata, selecione **Sair**.
1. Na janela **Desconectada**, selecione **Fechar**.
...
1. na lista de aplicativos, clique duas vezes em **Prompt de Comando**, quando solicitado, forneça a senha da conta **aduser1** e verifique se uma janela do **Prompt de Comando** é aberta com êxito.
...
1. Na janela **Prompt de Comando**, digite **logoff** e pressione a tecla **Enter** para sair da sessão da Área de Trabalho Remota.
1. Na sessão do Bastion para **az140-cl-vm11**, na janela do cliente da **Área de Trabalho Remota**, na lista de aplicativos, clique duas vezes em **SessionDesktop** em az140-21-ws1 e verifique se uma sessão da Área de Trabalho Remota é iniciada. 
...
1. Verifique se as configurações da janela do **Prompt de Comando** correspondem àquelas que você configurou anteriormente nesta tarefa.
...
1. Volte à sessão do Bastion para **az140-cl-vm11**, na janela do cliente da **Área de Trabalho Remota**, na lista de aplicativos e clique duas vezes em **SessionDesktop** para iniciar uma nova sessão da Área de Trabalho Remota. 
1. Na sessão **SessionDesktop**, verifique se o atalho do **Bloco de Notas** aparece na área de trabalho.
1. Na sessão **SessionDesktop**, clique com o botão direito do mouse em **Iniciar**, no menu do botão direito do mouse, selecione **Desligar ou sair** e, em seguida, no menu em cascata, selecione **Sair**.
1. Alterne para o computador do laboratório e, na janela do Microsoft Edge que exibe o portal do Azure, navegue até a folha **Contas de armazenamento** e selecione a entrada que representa a conta de armazenamento criada no exercício anterior.
1. Na folha da conta de armazenamento, na seção **Serviços de arquivo**, selecione **Compartilhamentos de arquivos** e, na lista de compartilhamentos de arquivos, selecione **az140-22-profiles**. 
1. Na folha **az140-22-profiles**, selecione **Procurar** e verifique se seu conteúdo inclui uma pasta cujo nome consiste em uma combinação do identificador de segurança (SID) da conta **ADATUM\\aduser1** seguido pelo sufixo **_aduser1**.
1. Selecione a pasta que você identificou na etapa anterior e observe que ela contém um único arquivo chamado **Profile_aduser1.vhd**.
